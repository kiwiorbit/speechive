...
const RotatedSquare: React.FC<{
  ...
  if (isUnlocked) {
    if (completedActivities >= 2) {
        colorClasses = "bg-rose-400 border-rose-400 text-white shadow-md";
    } else {
        colorClasses = "bg-rose-50 border-rose-400 text-rose-900";
    }
  } else {
...
const RecastPage: React.FC<RecastPageProps> = ({ challenge, setActivePage, onDaySelect }) => {
  ...
  return (
    <div className="flex flex-col h-full animate-fadeIn relative overflow-hidden">
      <div className="flex-shrink-0 mb-4 mt-2 px-1">
         <div className="bg-rose-50 border-l-4 border-rose-400 p-4 rounded-r-xl shadow-sm flex items-center justify-between">
             <div className="flex items-center">
                 <i className="fas fa-retweet text-rose-500 text-2xl mr-3"></i>
                 <div>
                     <p className="text-rose-800 font-bold text-base md:text-xl leading-tight">
                        Correct grammatical errors in a positive, conversational way.
                     </p>
                 </div>
             </div>
             <button 
                onClick={() => setShowInfo(true)}
                className="ml-3 w-8 h-8 flex items-center justify-center rounded-full bg-white/60 text-rose-600 hover:bg-white transition-colors focus:outline-none flex-shrink-0"
                aria-label="More info about Recast"
             >
           ...
           <div className="bg-white w-full max-w-md rounded-t-3xl sm:rounded-2xl p-6 sm:p-8 pb-20 text-left animate-slide-up-bottom relative z-10 shadow-2xl max-h-[85vh] overflow-y-auto">
              ...
              <div className="flex items-center space-x-3 mb-6">
                 <div className="w-12 h-12 rounded-full bg-rose-100 flex items-center justify-center flex-shrink-0">
                    <i className="fas fa-retweet text-rose-500 text-2xl"></i>
                 </div>
                 <h2 className="text-2xl font-bold text-gray-800">What is Recast?</h2>
              </div>
              
              <div className="space-y-4 text-gray-600">
                  <p className="leading-relaxed">
                    Repeating the child’s real word(s), adding one or more words, and rephrasing it as a question.
                  </p>
                  
                  <div className="bg-rose-50 p-5 rounded-2xl border border-rose-100 mt-4">
                    <h4 className="font-bold text-rose-800 mb-3 uppercase text-xs tracking-wider">Example</h4>
                    <ul className="space-y-4">
                        <li className="flex gap-3">
                           <div className="w-6 h-6 rounded-full bg-rose-200 text-rose-800 flex items-center justify-center font-bold text-xs flex-shrink-0 mt-0.5">1</div>
                           <p className="text-sm">Child: <span className="font-bold text-gray-800">“egg.”</span></p>
                        </li>
                        <li className="flex gap-3">
                           <div className="w-6 h-6 rounded-full bg-rose-200 text-rose-800 flex items-center justify-center font-bold text-xs flex-shrink-0 mt-0.5">2</div>
                           <p className="text-sm">Adult: <span className="font-bold text-gray-800">“Do you want the egg?”</span></p>
                        </li>
                    </ul>
                  </div>
              </div>
              ...
              <button 
                onClick={() => setShowInfo(false)}
                className="w-full mt-8 py-3.5 bg-rose-500 hover:bg-rose-600 text-white font-bold rounded-xl shadow-lg transition-all"
              >
                Got it
              </button>
           </div>
