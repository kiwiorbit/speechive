...
const InfoRow: React.FC<{ label: string; value: string }> = ({ label, value }) => (
    <div className="flex justify-between md:justify-start items-start">
        ...
        <span className="text-gray-800 font-medium text-sm text-right md:text-left">{value}</span>
    </div>
);

const Tag: React.FC<{ text: string; color: string }> = ({ text, color }) => (
    <span className={`px-3 py-1 rounded-lg text-xs font-semibold border ${color}`}>{text}</span>
);
...
const UserInfoHeader: React.FC<{ userInfo: UserInfo }> = ({ userInfo }) => {
    ...
    const calculateAge = (dobString: string): string => {
        ...
        try {
            ...
            return `${formattedDob} (${months} mths, ${days} days)`;
        } catch (e) {
            return 'Invalid Date';
        }
    };
    ...
    return (
        <Card id="user-info-report" className="!p-0 overflow-hidden bg-white border border-gray-100 shadow-sm rounded-xl">
            <div className="p-4 flex justify-between items-center bg-gray-50 border-b border-gray-100">
                <div className="flex items-center space-x-3">
                    <div className="w-10 h-10 rounded-full bg-amber-100 flex items-center justify-center shadow-sm">
                        <i className="fas fa-child text-amber-500 text-lg"></i>
                    </div>
                    <div>
                        <h3 className="text-lg font-bold text-gray-800">Participant Profile</h3>
                        {!isVisible && <p className="text-xs text-gray-500">{userInfo.childName}</p>}
                    </div>
                </div>
                <button 
                    onClick={() => setIsVisible(!isVisible)}
                    className="text-gray-400 hover:text-amber-500 transition-colors p-2 rounded-full hover:bg-gray-200"
                    aria-label={isVisible ? "Hide profile" : "Show profile"}
                >
                    <i className={`fas ${isVisible ? 'fa-eye-slash' : 'fa-eye'} text-lg`}></i>
                </button>
            </div>
            
            {isVisible && (
                <div className="p-4 md:p-6 space-y-6 animate-slideUpFadeIn">
                    {/* Basic Info Grid */}
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                        {/* Column 1: Child Details */}
                        <div>
                            <div className="flex items-center space-x-2 mb-3">
                                <i className="fas fa-id-card text-sky-400 text-sm"></i>
                                <h4 className="text-xs font-bold text-gray-400 uppercase tracking-wider">Child Details</h4>
                            </div>
                            <div className="space-y-3 pl-2">
                                <InfoRow label="Name" value={userInfo.childName} />
                                <InfoRow label="Age/DOB" value={calculateAge(userInfo.childDob)} />
                                <InfoRow label="Primary Language" value={userInfo.homeLanguage} />
                                {userInfo.additionalLanguages && <InfoRow label="Other Languages" value={userInfo.additionalLanguages} />}
                            </div>
                        </div>

                        {/* Column 2: Parent/Background */}
                        <div>
                            <div className="flex items-center space-x-2 mb-3">
                                <i className="fas fa-user-friends text-rose-400 text-sm"></i>
                                <h4 className="text-xs font-bold text-gray-400 uppercase tracking-wider">Background</h4>
                            </div>
                            <div className="space-y-3 pl-2">
                                <InfoRow label="Caregiver Education" value={userInfo.parentalEducation} />
                                <InfoRow label="Referral Source" value={userInfo.referralSource || 'Not specified'} />
                                <InfoRow label="Research Consent" value={userInfo.researchConsent ? 'Given' : 'Not Given'} />
                                {userInfo.role === 'participant' && (
                                    <InfoRow label="Talked to Researcher" value={userInfo.researchDiscussion ? 'Yes' : 'No'} />
                                )}
                            </div>
                        </div>
                    </div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                         <div>
                            <div className="flex items-center space-x-2 mb-3">
                                <i className="fas fa-bullseye text-indigo-400 text-sm"></i>
                                <h4 className="text-xs font-bold text-gray-400 uppercase tracking-wider">Goals</h4>
                            </div>
                            <div className="flex flex-wrap gap-2 pl-2">
                                {userInfo.parentGoals.length > 0 ? (
                                    userInfo.parentGoals.map(g => <Tag key={g} text={g} color="bg-indigo-50 text-indigo-700 border-indigo-100" />)
                                ) : <span className="text-sm text-gray-400 italic">No goals selected</span>}
                            </div>
                         </div>
                         <div>
                            <div className="flex items-center space-x-2 mb-3">
                            ...
                            <div className="flex flex-wrap gap-2 pl-2">
                                {userInfo.childInterests.length > 0 ? (
                                    userInfo.childInterests.map(i => <Tag key={i} text={i} color="bg-pink-50 text-pink-700 border-pink-100" />)
                    ...
                    {userInfo.hearingHistory && (
                        <>
                            <div className="border-t border-gray-100"></div>
                            <div>
                                <div className="flex items-center space-x-2 mb-3">
                                    <i className="fas fa-ear-listen text-orange-400 text-sm"></i>
                                    <h4 className="text-xs font-bold text-gray-400 uppercase tracking-wider">Hearing Profile</h4>
                                </div>
                                <div className="pl-2">
                                    {userInfo.hearingHistory.hasHearingLoss ? (
                                        <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                                            <InfoRow label="Hearing Loss" value={userInfo.hearingHistory.degree || 'Not specified'} />
                                            <InfoRow label="Devices" value={userInfo.hearingHistory.hasDevices ? (userInfo.hearingHistory.deviceTypes?.join(', ') || 'Yes') : 'None'} />
                                            {userInfo.hearingHistory.hearingAidConfig && <InfoRow label="Hearing Aids" value={userInfo.hearingHistory.hearingAidConfig} />}
                                            {userInfo.hearingHistory.cochlearImplantConfig && <InfoRow label="Cochlear Implants" value={userInfo.hearingHistory.cochlearImplantConfig} />}
                                        </div>
                                    ) : (
                                        <p className="text-sm text-gray-600">Typical Hearing</p>
                                    )}
                                </div>
                            </div>
                        </>
