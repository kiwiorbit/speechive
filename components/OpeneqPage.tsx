...
const RotatedSquare: React.FC<{
  ...
  if (isUnlocked) {
    if (completedActivities >= 2) {
        colorClasses = "bg-indigo-400 border-indigo-400 text-white shadow-md";
    } else {
        colorClasses = "bg-indigo-50 border-indigo-400 text-indigo-900";
    }
  } else {
...
const OpenEQPage: React.FC<OpenEQPageProps> = ({ challenge, setActivePage, onDaySelect }) => {
  ...
  return (
    <div className="flex flex-col h-full animate-fadeIn relative overflow-hidden">
      <div className="flex-shrink-0 mb-4 mt-2 px-1">
         <div className="bg-indigo-50 border-l-4 border-indigo-400 p-4 rounded-r-xl shadow-sm flex items-center justify-between">
             <div className="flex items-center">
                 <i className="fas fa-question-circle text-indigo-500 text-2xl mr-3"></i>
                 <div>
                     <p className="text-indigo-800 font-bold text-base md:text-xl leading-tight">
             ...
             <button 
                ...
                className="ml-3 w-8 h-8 flex items-center justify-center rounded-full bg-white/60 text-indigo-600 hover:bg-white transition-colors focus:outline-none flex-shrink-0"
                aria-label="More info about Open EQ"
             >
        ...
        <div className="fixed inset-0 z-50 flex items-end justify-center sm:items-center">
           ...
           <div className="bg-white w-full max-w-md rounded-t-3xl sm:rounded-2xl p-6 sm:p-8 pb-20 text-left animate-slide-up-bottom relative z-10 shadow-2xl max-h-[85vh] overflow-y-auto">
              ...
              <div className="flex items-center space-x-3 mb-6">
                 ...
                 <h2 className="text-2xl font-bold text-gray-800">What are Open-Ended Questions?</h2>
              </div>
              
              <div className="space-y-4 text-gray-600">
                  <p className="leading-relaxed">
                    Questions that cannot be answered with “yes” or “no” and encourage the child to express ideas, actions, or choices.
                  </p>
                  
                  <div className="bg-indigo-50 p-5 rounded-2xl border border-indigo-100 mt-4">
                    <h4 className="font-bold text-indigo-800 mb-3 uppercase text-xs tracking-wider">When using open-ended questions:</h4>
                    <ul className="space-y-4">
                        <li className="flex gap-3">
                           <div className="w-6 h-6 rounded-full bg-indigo-200 text-indigo-800 flex items-center justify-center font-bold text-xs flex-shrink-0 mt-0.5">1</div>
                           <p className="text-sm">Ask a question based on the child’s focus or activity, e.g., <span className="font-bold text-gray-800">“What are you building?”</span> or <span className="font-bold text-gray-800">“Which block do you want next?”</span></p>
                        </li>
                        <li className="flex gap-3">
                           <div className="w-6 h-6 rounded-full bg-indigo-200 text-indigo-800 flex items-center justify-center font-bold text-xs flex-shrink-0 mt-0.5">2</div>
                           <p className="text-sm">Wait at least 5 seconds to give the child sufficient time to respond before providing a model, or correction.</p>
                        </li>
                    </ul>
                  </div>
              </div>
              ...
              <button 
                onClick={() => setShowInfo(false)}
                className="w-full mt-8 py-3.5 bg-indigo-500 hover:bg-indigo-600 text-white font-bold rounded-xl shadow-lg transition-all"
              >
                Got it
              </button>
           </div>
